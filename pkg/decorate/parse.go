...
// sourceMode generates audits via source file.
...
func parseSkip(file *ast.File) map[string]bool {
	rt := map[string]bool{}
	for _, v := range file.Comments {
		for _, c := range v.List {
			if strings.HasPrefix(c.Text, "//igen:skip") {
				arr := strings.Split(strings.Replace(c.Text, "//igen:skip", "", -1), ",")
				for _, v := range arr {
					rt[strings.Replace(v, " ", "", -1)] = true
				}
			}
		}
	...
	for _, field := range it.it.Methods.List {
		...
		case *ast.SelectorExpr:
			...
			if embeddedIfaceType != nil {
				...
				if embeddedIfaceType = parser.importedInterfaces.Get(filePkg, sel); embeddedIfaceType == nil {
		...
		default:
			return nil, fmt.Errorf("don't know how to audit method of type %T", field.Type)
		}
	}
	return iface, nil
}
